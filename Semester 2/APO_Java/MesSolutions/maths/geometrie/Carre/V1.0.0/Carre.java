...
//
// + Version 0.0.0 : TP5 constructeur, accesseurs, toString, redimension, clone, equals
...
private double m_cote=getLargeur();
	...
	public void redimension(double x)throws Exception{super.redimension(x,x);}
	
	// ---										Methode contient
	
	public boolean contient (double x, double y)	{//return super.contient(x,y);
	
		return  getPoint().getAbscisse()<= x && x<= getPoint().getAbscisse()+getCote()  &&
				 getPoint().getOrdonnee()<= y && y<= getPoint().getOrdonnee()+getCote() ;
	}
