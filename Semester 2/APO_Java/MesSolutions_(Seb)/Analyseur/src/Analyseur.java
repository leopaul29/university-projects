...
public class Analyseur
{
  ...
  {
  	Field [] attribut = m_cl.getDeclaredFields();
  	
  	for(Field e : attribut)		//voilà
  	{
  ...
  public Object creerInstance(Object ... valParam)	throws InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException
  {
  	Object o;
    
	if( m_typeParam.length == 0)
  		return null;
	else 
	{
		...
		{
			Class<?> novalues[] =  new  Class<?> [m_typeParam.length] ;
			int i = 0;
			
			for(Class <?> e : m_typeParam)
			{
				novalues[i] = e;
				i++;
			}
			...
			o = m_constructor.newInstance(valParam);
		}
  ...
  {
	 return this.m_method.invoke(o, args);
  }
